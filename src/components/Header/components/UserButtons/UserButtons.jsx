...
const UserButtons = ({headerIsLight}) => {
  ...
  return (
    <div className={`${s.wrapper} ${s[darkModeClass]}`}>
      <Link className={s.btn} to=''>Sell</Link>
      <Link className={s.login} to={routes.login}>Login</Link>
      <button className={s.favorite} type="button" aria-label='favorite'>
        <Icon name="favorite" fill='white' classNameInner={s.favoriteInner} classNameOver={s.favoriteOver} />
